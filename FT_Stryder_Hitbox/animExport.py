...
def export_anim(file, generate_multiple_files, obj, obj_index=0, source_name='', source_address='clamp', parent_name='',
    ...
    rot_angles_Z2 = 0.0
    i=0
    armature=bpy.data.objects["Stryder_Rig"]
    with open(file_path, 'w') as cfg:
        if (selection_name == "deef_c_hip") or (selection_name == "jea_c_propGun"):
            for f in frames:
                ...
                if last is not None:
                    ...
                    if round(length, precision) == 0:
                        d = Vector((1, 0, 0))
                    dn = d.normalized()
                    rot_angles0 = armature.pose.bones[selection_name].location
                    rot_angles = rot_angles0
                    rot_angles_X = (rot_angles[0])
                    rot_angles_Y = (rot_angles[1])
                    rot_angles_Z = (rot_angles[2])
                    rot_angles_X = round(rot_angles_X,3)
                    rot_angles_Y = round(rot_angles_Y,3)
                    rot_angles_Z = round(rot_angles_Z,3)
                    rot_angles_X1 = (rot_angles_X-rot_angles_X2)*4.4641
                    rot_angles_Y1 = (rot_angles_Y-rot_angles_Y2)*1.7837
                    rot_angles_Z1 = -(rot_angles_Z-rot_angles_Z2)*4.4629
                    rot_angles_X2 = rot_angles_X
                    rot_angles_Y2 = rot_angles_Y
                    rot_angles_Z2 = rot_angles_Z
                    q_axis = q.axis.normalized()
                    q_angle = degrees(q.angle)
                    comma=""
                    if round(q_angle, precision) == 0:
                        q_axis = Vector((1, 0, 0))
                    curr_min_value = (0.033 * i) + 0.066
                    curr_max_value = (0.033 * (i)) + 0.099
                    if (i == 0):
                        curr_min_value = 0
                        curr_max_value = 0.001
                    if ((rot_angles_X1 > 0.002) or (rot_angles_X1 < -0.002)):
                        cfg.write(f"class {sanitize_classname(source_name)}_{sanitize_classname(selection_name)}_transalationX_{i} {{\n"
                                f"    type       = \"translation\";\n"
                                f"    source     = \"{source_name}\";\n"
                                f"    selection  = \"{selection_name}1\";\n"
                                f"    axis = \"{selection_name}1_axisX\";\n"
                                ...
                                f"    offset0      = 0;\n"
                                f"    offset1      = {rot_angles_X1:.{precision}f};\n"
                                f"}};\n")
                    i += 1
                last = curr.copy()
            i=0
            ...
            for f in frames:
                ...
                if last is not None:
                    ...
                    if (i == 0):
                        curr_min_value = 0
                        curr_max_value = 0.001
                    if ((rot_angles_Y1 > 0.002) or (rot_angles_Y1 < -0.002)):
                        cfg.write(f"class {sanitize_classname(source_name)}_{sanitize_classname(selection_name)}_transalationY_{i} {{\n"
                                f"    type       = \"translation\";\n"
                                f"    source     = \"{source_name}\";\n"
                                f"    selection  = \"{selection_name}1\";\n"
                                f"    axis = \"{selection_name}1_axisY\";\n"
                                f"{f'    sourceAddress = {source_address};{new_line_char}' if source_address != 'clamp' else ''}"
                                f"    minValue   = {curr_min_value:.{precision}f};\n"
                                f"    maxValue   = {curr_max_value:.{precision}f};\n"
                                f"    offset0      = 0;\n"
                                f"    offset1      = {rot_angles_Y1:.{precision}f};\n"
                                f"}};\n")
                    i += 1
                last = curr.copy()
            i=0
            ...
            for f in frames:
                ...
                if last is not None:
                    ...
                    if (i == 0):
                        curr_min_value = 0
                        curr_max_value = 0.001
                    if ((rot_angles_Z1 > 0.002) or (rot_angles_Z1 < -0.002)):
                        cfg.write(f"class {sanitize_classname(source_name)}_{sanitize_classname(selection_name)}_transalationZ_{i} {{\n"
                                f"    type       = \"translation\";\n"
                                f"    source     = \"{source_name}\";\n"
                                f"    selection  = \"{selection_name}1\";\n"
                                f"    axis = \"{selection_name}1_axisZ\";\n"
                                f"{f'    sourceAddress = {source_address};{new_line_char}' if source_address != 'clamp' else ''}"
                                f"    minValue   = {curr_min_value:.{precision}f};\n"
                                f"    maxValue   = {curr_max_value:.{precision}f};\n"
                                f"    offset0      = 0;\n"
                                f"    offset1      = {rot_angles_Z1:.{precision}f};\n"
                                f"}};\n")
                    i += 1
                last = curr.copy()
            i=0
        ...
        rot_angles_Y2 = 0.0
        rot_angles_Z2 = 0.0
        if (selection_name == "Hip_L") or (selection_name == "Hip_R") or (selection_name == "Thigh_L") or (selection_name == "Thigh_R") or (selection_name == "ThighLow_R") or (selection_name == "ThighLow_L") or (selection_name == "Knee_L") or (selection_name == "Knee_R") or (selection_name == "Ankle_L") or (selection_name == "Ankle_R") or (selection_name == "Spine_A") or (selection_name == "Spine_B") or (selection_name == "Clav_L") or (selection_name == "Clav_R") or (selection_name == "Shoulder_L") or (selection_name == "Shoulder_R") or (selection_name == "Elbow_L") or (selection_name == "Elbow_R") or (selection_name == "Wrist_L") or (selection_name == "Wrist_R"):
            for f in frames:
                bpy.context.scene.frame_set(f)
                if parent_object is not None:
                    parent_matrix = parent_object.matrix_world
                ...
                if last is not None:
                    ...
                    if round(length, precision) == 0:
                        d = Vector((1, 0, 0))
                    dn = d.normalized()
                    rot_angles0 = armature.pose.bones[selection_name].rotation_quaternion
                    rot_angles = rot_angles0.to_euler()
                    rot_angles_X = degrees(rot_angles[0])
                    rot_angles_Y = degrees(rot_angles[1])
                    rot_angles_Z = degrees(rot_angles[2])
                    rot_angles_X = round(rot_angles_X,3)
                    rot_angles_Y = round(rot_angles_Y,3)
                    rot_angles_Z = round(rot_angles_Z,3)
                    rot_angles_Y = round(rot_angles_Y,3)
                    rot_angles_Z = round(rot_angles_Z,3)
                    rot_angles_X1 = -(rot_angles_X-rot_angles_X2)
                    rot_angles_Y1 = -(rot_angles_Y-rot_angles_Y2)
                    rot_angles_Z1 = (rot_angles_Z-rot_angles_Z2) 
                    rot_angles_X2 = rot_angles_X
                    rot_angles_Y2 = rot_angles_Y
                    rot_angles_Z2 = rot_angles_Z
                    q_axis = q.axis.normalized()
                    ...
                    if (i == 0):
                        ...
                        curr_max_value = 0.001
                    if ((rot_angles_X1 > 0.002) or (rot_angles_X1 < -0.002)):
                        cfg.write(f"class {sanitize_classname(source_name)}_{sanitize_classname(selection_name)}_rotationX_{i} {{\n"
                                f"    type       = \"rotation\";\n"
                                f"    source     = \"{source_name}\";\n"
                                f"    selection  = \"{selection_name}\";\n"
                                f"    axis = \"{selection_name}_axisX\";\n"
                                f"{f'    sourceAddress = {source_address};{new_line_char}' if source_address != 'clamp' else ''}"
                                f"    minValue   = {curr_min_value:.{precision}f};\n"
                                f"    maxValue   = {curr_max_value:.{precision}f};\n"
                                f"    angle0      = 0;\n"
                                f"    angle1      = \"rad {rot_angles_X1:.{precision}f}\";\n"
                                f"}};\n")
                    i += 1
                last = curr.copy()
            i=0
            ...
            for f in frames:
                ...
                if last is not None:
                    ...
                    if (i == 0):
                        ...
                        curr_max_value = 0.001
                    if ((rot_angles_Y1 > 0.002) or (rot_angles_Y1 < -0.002)):
                        cfg.write(f"class {sanitize_classname(source_name)}_{sanitize_classname(selection_name)}_rotationY_{i} {{\n"
                                f"    type       = \"rotation\";\n"
                                f"    source     = \"{source_name}\";\n"
                                f"    selection  = \"{selection_name}\";\n"
                                f"    axis = \"{selection_name}_axisY\";\n"
                                f"{f'    sourceAddress = {source_address};{new_line_char}' if source_address != 'clamp' else ''}"
                                f"    minValue   = {curr_min_value:.{precision}f};\n"
                                f"    maxValue   = {curr_max_value:.{precision}f};\n"
                                f"    angle0      = 0;\n"
                                f"    angle1      = \"rad {rot_angles_Y1:.{precision}f}\";\n"
                                f"}};\n")
                    i += 1
                last = curr.copy()
            i=0
            ...
            for f in frames:
                ...
                if last is not None:
                    ...
                    rot_angles_Y = degrees(rot_angles[1])
                    rot_angles_Z = degrees(rot_angles[2])
                    rot_angles_X = round(rot_angles_X,1)
                    rot_angles_Y = round(rot_angles_Y,1)
                    rot_angles_Z = round(rot_angles_Z,1)
                    rot_angles_X1 = -(rot_angles_X-rot_angles_X2)
                    rot_angles_Y1 = -(rot_angles_Y-rot_angles_Y2)
                    rot_angles_Z1 = (rot_angles_Z-rot_angles_Z2) 
                    rot_angles_X2 = rot_angles_X
                    ...
                    if (i == 0):
                        ...
                        curr_max_value = 0.001
                    if ((rot_angles_Z1 > 0.002) or (rot_angles_Z1 < -0.002)):
                        cfg.write(f"class {sanitize_classname(source_name)}_{sanitize_classname(selection_name)}_rotationZ_{i} {{\n"
                                f"    type       = \"rotation\";\n"
                                f"    source     = \"{source_name}\";\n"
                                f"    selection  = \"{selection_name}\";\n"
                                f"    axis = \"{selection_name}_axisZ\";\n"
                                f"{f'    sourceAddress = {source_address};{new_line_char}' if source_address != 'clamp' else ''}"
                                f"    minValue   = {curr_min_value:.{precision}f};\n"
                                f"    maxValue   = {curr_max_value:.{precision}f};\n"
                                f"    angle0      = 0;\n"
                                f"    angle1      = \"rad {rot_angles_Z1:.{precision}f}\";\n"
                                f"}};\n")
                    i += 1
                last = curr.copy()
    return (0, i)
